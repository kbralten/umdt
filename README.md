...
1. **Interactive Tool**: A CLI and GUI for reading/writing registers, scanning addresses, probing networks, and live monitoring.
2. **Mock Server**: A configurable simulation environment for creating virtual Modbus devices with fault injection and scripting.
3. **Bridge**: A soft-gateway for routing Modbus traffic between TCP and Serial (RTU) transports, with PCAP logging and scripting hooks.
4. **Sniff**: Lightweight CLI and GUI sniffers for capturing Modbus traffic (TCP/RTU) to PCAP files compatible with the UMDT Wireshark wrapper.
